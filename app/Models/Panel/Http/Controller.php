...
class Controller extends \App\Http\Controller
{
    ...
    public function __reconstruct()
    {
        ...
        $className = get_class_name($controllerClassName);

        // Set Model
        if (empty($this->model)) {
            $modelClassName = str_replace(['Panel\Controllers', 'Controllers'], ['Api\Models', 'Models'], $controllerClassName);
        ...
        if ($modules->count()) {
            foreach ($modules as $module) {
                ...
                if (is_dir($modulePath)) {
                    // Load Navigations
                    $navigationsConfigFilePath = $modulePath . 'Config' . DIRECTORY_SEPARATOR . 'Navigations.php';
                    if (is_file($navigationsConfigFilePath)) {
                        include_once($navigationsConfigFilePath);
